...
> For CESR encoding theory, code table values, Sizage formulas, and SAID algorithms, see the **cesr-spec** skill. This file covers the TypeScript API only.
...
**Codex enums:** MtrDex, NonTransDex, DigiDex, NumDex, BexDex, SmallVrzDex, LargeVrzDex, IdrDex, IdxSigDex, IdxCrtSigDex, IdxBthSigDex — see cesr-spec for code point registry.
...
### Sizage / Xizage
`class Sizage(hs, ss, fs?, ls?)` — see cesr-spec for field semantics.
